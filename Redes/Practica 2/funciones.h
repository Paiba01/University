...
int buscarPartida(int socket){
	for (int i = 0; i < 10; i++){
		if(games[i].state==INACTIVA && games[i].players[0]!=-1){ // Si la partida esta inactiva y hay un jugador en la mesa
	...
	for (int i = 0; i < 10; i++){
		if(games[i].state==INACTIVA && games[i].players[0]==-1){ // Si la partida esta inactiva y no hay ningún jugador en la mesa
